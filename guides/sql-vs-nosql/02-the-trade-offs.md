...
and the weaknesses of the side they don't. We're going to do it the honest way: each trade-off
gets both sides, because every one of these is a *trade* — you gain something and you give
something up. There's no free lunch, only lunches you're choosing to pay for differently.

Here's the whole phase in one table. Read it, then read the sections under it for the *why*.
...
*What just happened:* The database refused a bad write before it could rot your data. You
didn't write that check in your app — the schema *is* the check. Multiply that across every
write from every part of your system, and you get data you can trust.
...
didn't disappear; it moved into your application code (more on that in Phase 3).
...
**normalized** (no duplicated data) while still answering "show me X with its related Y."
...
**The relational side.** A traditional relational database runs on one primary server and
offers **strong consistency**: once a write succeeds, every following read sees it. There's one
authoritative copy, so there's no ambiguity about "the current value." This is wonderful for
anything where being wrong for even a moment matters — money, inventory, bookings. The limit is
that a single machine can only get so big; scaling *up* (a bigger server) has a ceiling, and
scaling *out* (spreading one logical database across many machines) is genuinely hard to do
while keeping that strong consistency.

**The NoSQL side.** Several NoSQL stores (wide-column especially) are built from day one to
spread across many machines — **horizontal scale**. Add servers, absorb more data and more
writes. That's how you handle volumes a single box can't. But spreading data across machines
forces a hard question: when a network hiccup splits your servers apart for a moment, do you
refuse writes (stay consistent) or keep accepting them and reconcile later (stay available)?
...
> availability — you have to favor one. That's the whole idea. Many distributed NoSQL stores
> favor availability and offer **eventual consistency**: a write will show up everywhere
> *soon*, but a read right after a write might briefly see the old value.

**The honest framing.** Strong consistency on one machine is easy and is the relational
default. Massive horizontal scale is easy for distributed NoSQL stores and is their default.
Getting *both at once* is the genuinely hard engineering problem, and it's why "which do you
need more — guaranteed-correct-right-now, or scale-past-one-machine?" is one of the most useful
questions you can ask about an app.
...
is rarely a reason to give up strong consistency *today*. Phase 3 comes back to this.
...
so there's almost no work to do at read time.

The catch is the flip side of that fit: ask a question the store *wasn't* shaped for, and it's
awkward or slow. "Find all sessions belonging to users in Canada" is trivial SQL and nearly
impossible to ask a plain key-value cache, because the cache only knows how to look up by key.
...
> real — but it's narrow, and "is it faster?" only means anything once you name the query.
> "NoSQL is faster than SQL" with no access pattern attached is not a true sentence; it's a
> bumper sticker.
