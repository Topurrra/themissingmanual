...
There are exactly two questions: **will it fit?** and **will it be fast enough?** Let's take them in order.
...
> 📝 **Terminology.** **Parameters** are the learned numbers that *are* the model. "A 7B model" means 7 billion of them. The "B" is the headline number on almost every open-weights model, and it's the first thing to look at.
...
⚠️ **Gotcha — the big one.** A giant model will not fit a small machine, period. If a model needs more memory than you have, it either refuses to load or spills onto disk and crawls so slowly it's unusable. Nobody is running a 70B model on an 8 GB laptop. Match the model to the machine *before* you pull it, not after.
...
> 📝 **Terminology.** **RAM** is your computer's main system memory. **VRAM** is the separate, faster memory built onto a graphics card (GPU). A model running on the GPU must fit in VRAM; a model running on the CPU uses ordinary RAM. (For the full picture of how memory works, see [CPU, RAM & Storage, Explained](/guides/cpu-ram-and-storage).)

VRAM is usually the tighter constraint, because consumer graphics cards ship with less of it than a machine has system RAM. A model that fits comfortably in 32 GB of RAM might be far too big for an 8 GB graphics card. When you're checking "will it fit," check against the memory it'll actually use.

## Quantization — shrinking the weights to fit

Here's the lever that makes local models practical, and it's the one concept most worth understanding in this whole guide.
...
A purely practical warning that has nothing to do with hardware.

⚠️ **Gotcha — open-weights licenses vary.** "Open-weights" means you can download and run the model; it does *not* automatically mean you can do anything you want with it. Some models are permissively licensed; others restrict commercial use, large-scale deployment, or specific applications, and some require you to accept terms before downloading. Before you build something real — especially anything commercial — read the license for the specific model. The weights being downloadable is not the same as the use being unrestricted.

## Matching a model to your machine

Put it all together as a quick mental checklist, in order:

1. **Start from your memory.** How much RAM do you have? Do you have a GPU, and how much VRAM? That's your budget.
2. **Pick a parameter size that fits the budget**, with room to spare — a model should fit *comfortably*, not exactly.
3. **Use a quantized version** (a `Q4`-style build is a sensible default) to stretch that budget further.
4. **Try it** — `ollama run` it, ask real questions. If it's good enough and fast enough, you're done.
5. **If it doesn't fit or it's too slow**, step down: smaller parameter count, or more aggressive quantization, or both.
6. **If quality isn't enough** even at the largest model you can fit, that's the honest signal to use a hosted API for that task — exactly the trade from [Phase 1](01-why-run-locally.md).
...
You can now read any model on a download page and predict whether it'll run on your machine, and why — which was the whole point. From here, the natural next steps are feeding a local model your own documents and, eventually, fine-tuning one — each its own guide.
