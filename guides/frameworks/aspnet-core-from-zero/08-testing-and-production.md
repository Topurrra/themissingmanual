...
1. `appsettings.json` — your base settings, committed to the repo.
...
- **Override services for tests** with `factory.WithWebHostBuilder(b => b.ConfigureServices(...))` — `RemoveAll<T>()` the real registration and add a fake/in-memory one. Unit tests of a single service or repository need no factory at all; just `new` it up with a fake dependency.
