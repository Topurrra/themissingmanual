...
The View binds to the ViewModel. You set the ViewModel as the page's `BindingContext`, and every `{Binding Title}` in the XAML reads from a `Title` property on the ViewModel. The arrows only point one way in your head: **the View asks the ViewModel for data; the ViewModel never reaches up and touches the View.**

> 💡 The whole payoff is in that last sentence. Because the ViewModel has **no UI dependency**, you can create one in a unit test, call its `AddNote` command, and assert that the notes collection grew — no screen, no emulator, no tapping. A ViewModel you can test without a UI is the entire point of MVVM. If you remember one thing, remember that.
...
A binding isn't magic. When you write `{Binding Title}`, MAUI reads `Title` once and shows it. For MAUI to *re-read* it when the value changes, the object holding `Title` has to **announce the change**. The contract for that announcement is an interface called `INotifyPropertyChanged` — one event, `PropertyChanged`, that you raise whenever a bound property's value changes.
...
Now look at how much ceremony that is for **one** property: a backing field, a hand-written getter, a hand-written setter, the event invocation, and the `nameof`. Imagine a notes page with a title, a body, a "saving…" flag, and a selected note. You'd write that pattern four times, and one typo in a `nameof` gives you a silent bug where the screen mysteriously won't refresh for that one field. Nobody enjoys this. So nobody writes it by hand anymore.

## The modern way: CommunityToolkit.Mvvm

`CommunityToolkit.Mvvm` is a small, official NuGet package that generates all that boilerplate for you at compile time using **source generators**. You add the package, inherit from `ObservableObject`, and annotate fields — the generator writes the properties and the `PropertyChanged` plumbing behind the scenes.
...
- `[ObservableProperty]` on the field `newTitle` generates a public property `NewTitle` (capital N) — *with* the getter, setter, and `PropertyChanged` notification from the hand-written version above. You write one line; the generator writes fifteen.
- `ObservableCollection<Note>` is a list that *itself* raises change notifications. When you `Add` to it, any bound list on screen updates automatically — no extra code. (A plain `List<Note>` would not; the UI wouldn't see new items.)
- `[RelayCommand]` on the method `AddNote` generates a public property `AddNoteCommand` of type `ICommand`. That's the thing your button will bind to.
...
That handler sits in the page's `.xaml.cs` file, which means your logic is tangled up with the View — exactly what MVVM is trying to separate. The MVVM way replaces the event with a **command binding**:
...
⚠️ Here's the trap, and it's the most common MVVM mistake: the moment something is mildly inconvenient, you reach for a UI call inside the ViewModel. "I'll just pop a `DisplayAlert` to confirm the delete." Don't.

`DisplayAlert`, navigation, and other UI calls live on the *page*, not the ViewModel. The second your ViewModel calls `DisplayAlert`, it can no longer be constructed in a unit test — it depends on a live page — and you've thrown away the entire benefit. The same goes for stuffing logic back into code-behind: if real decisions live in `.xaml.cs`, you can't test them either.
...
When you find yourself wanting UI in the ViewModel, that's the signal to extract a service — not to break the boundary. Guard that boundary and your ViewModels stay the cheap-to-test core of your app, exactly as designed.
