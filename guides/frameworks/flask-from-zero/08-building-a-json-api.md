...
If "GET, POST, paths, status codes, resources" aren't yet second nature, read [REST APIs explained](/guides/rest-apis-explained) alongside this — it's the vocabulary the whole phase leans on.
...
*What just happened:* this fetches a `Note` (the model from [Phase 5](05-database-with-sqlalchemy.md)) and renders it into an HTML template — a full page with markup, styling, the works. Perfect for a browser, useless to a program that just wants the title and content as data.
