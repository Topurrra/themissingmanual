...
Your stack comes up. So why does the API sometimes crash on startup, where exactly does your database's data live, and how do you get code changes to show up without rebuilding the image every time? These three questions — networking, persistence, and the dev loop — are what separate "I got the example running" from "I run my projects on this."

This phase answers all three, and then names the two traps that bite people once they're comfortable: `depends_on` not meaning what you think, and the temptation to ship your dev compose file straight to production.

## How services find each other: the Compose network

**What it actually is.** When Compose brings up a stack, it creates a private network and puts every service on it. On that network, **each service is reachable by its service name as a hostname.** The service called `db` in your file is literally reachable at the address `db` from any other service in the stack.
...
*What just happened:* `docker compose exec api ...` ran a command *inside* the running `api` container. From there, the name `db` resolved to the database container's address on the private network — no IP addresses written anywhere, no extra configuration. That name-based resolution is what makes the connection strings in your file work.
...
**What it actually is.** You met named volumes in Phase 2. Here's why they matter, made concrete. A container's own filesystem dies with the container. The named volume `db_data` lives outside the container, so when you recreate the database container, the data is still there.

**What it does in real life.** Add some data, recreate the container, and confirm it survived:
...
**What it actually is.** A named volume is storage Docker manages. A **bind-mount** is different: it maps a folder *on your machine* directly into the container, so the container sees your real, live source files. Edit a file in your editor and the container sees the change instantly — no image rebuild.
...
**Why this exists.** Without it, your workflow during development is brutal: change one line, rebuild the image, recreate the container, see the change. With a bind-mount plus a dev server that watches for file changes, you change a line and it's live in the container immediately.

**What it does in real life.** You add a bind-mount to the service running your code:
...
**The real fix: a healthcheck plus a condition.** You teach Compose how to know the database is genuinely ready, then make the API wait for *that*:
...
*What just happened:* The `healthcheck` runs `pg_isready` — Postgres's own "are you ready for connections?" probe — every 5 seconds until it succeeds, at which point the `db` service is marked *healthy*. The longer `depends_on` form, `condition: service_healthy`, tells Compose to hold the API back until `db` reports healthy, not just started. Now the API only launches once the database can actually answer it.
...
**What it actually is.** The file we've built is excellent for development and *wrong* for production, on purpose. The very things that make local dev pleasant are the things you must not ship.
...
**The calm way to handle it.** Keep the shared, true-everywhere parts in `docker-compose.yml`, and put the dev-only conveniences (bind-mounts, exposed debug ports, the throwaway password) in a separate `docker-compose.override.yml` that Compose merges in automatically *only* during local development. Production runs the base file with its own production settings layered on instead. The mental model is what matters here: **one file is not meant to serve both worlds** — keep the differences explicit. (The full override mechanics and a production-grade setup are the kind of deeper material a follow-up guide covers; for now, just don't deploy the dev file as-is.)
