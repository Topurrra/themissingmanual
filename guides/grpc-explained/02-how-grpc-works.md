...
In Phase 1 you got the mental model: define the functions once, both sides agree. Now let's make it
concrete. There are really only four moving parts, and once you've seen each one, the whole thing stops
...
*What just happened:* You declared a data shape (`ConvertRequest`), a result shape (`Money`), and one
function (`Convert`) that takes the first and returns the second. That's the entire contract. Any service
that has this file knows exactly how to call `Convert` and exactly what it gets back.

A few things that confuse people the first time:
...
*What just happened:* From one `currency.proto`, the compiler wrote two files of Go code. `currency.pb.go`
holds the message types (`ConvertRequest`, `Money`) as real Go structs. `currency_grpc.pb.go` holds the
**stubs** — the client and server scaffolding for the `Convert` function. The exact filenames and flags
differ per language (you'd pass `--python_out`, `--java_out`, and so on), but the idea is identical
everywhere.
...
This is the payoff of contract-first. Because both stubs come from the same `.proto`, and because you can
generate them in *different languages from the same file*, a Go service and a Python service can call each
other with full type safety on both ends — no hand-written serialization, no drift.

Here's the client side in practice (Go, trimmed to the essentials):
...
*What just happened:* You called `client.Convert(...)` as if `Convert` lived in your own program. Under the
hood, the stub serialized your request to binary, sent it over the open HTTP/2 connection to the currency
service, and deserialized the reply into a `Money` you can read with `resp.Amount`. The remote call is
hidden behind an ordinary-looking function call — that's the "Remote Procedure Call" idea made real.
...
*What just happened:* Protocol Buffers writes each field as its *number* (the `= 1` tag), a hint of its
type, and the raw value — and nothing else. The field names and JSON punctuation aren't there at all. The
result is meaningfully smaller and faster to read, because the receiver already knows from the contract what
field `1` means and doesn't have to parse any text. (The exact byte counts depend on the values; the point
is "fewer bytes and no text parsing," not a fixed ratio.)
...
*What just happened:* `WatchPrice` returns `stream Quote` instead of a single `Quote`, so the generated
client stub gives you something you read from in a loop as new quotes arrive, rather than a single value.
The contract itself records that this call streams — both sides generate the right shape of code for it.
