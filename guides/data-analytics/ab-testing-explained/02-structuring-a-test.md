...
Why one, and why in advance? Because if you watch ten metrics at once, at least one of them will move by chance even if your change does nothing at all — and it's tempting to declare *that* one "the real result" after the fact. Phase 3 covers this trap in detail (it's the most common way teams mislead themselves), but the prevention starts here: write down the metric before you see any data, and that's the number you judge the test by, full stop.
...
Say you run a test for one day, 40 people per group, and control converts at 10% while the variant converts at 15%. Exciting — except with 40 people, that's roughly 4 conversions vs. 6. A single person switching groups would change the whole story. Small samples swing wildly for the same reason a coin flipped 4 times can land heads 3 times without the coin being unfair — there just isn't enough data yet for randomness to average itself out.

This is **noise**: the natural bounce in a metric that happens even when nothing real is going on underneath. The larger your sample, the smaller that natural bounce gets relative to the size of a real effect, and the more you can trust that a gap you're seeing reflects something real rather than the equivalent of a lucky coin flip.
...
It is *not* a claim that the result is important, large, or permanent. It's a narrower claim: "if there were truly no difference between these two versions, we probably wouldn't have seen a gap this big by chance alone." That's it. A tiny, meaningless improvement can be "statistically significant" with enough users. A genuinely large improvement can fail to reach significance if you tested on too few people. Significance is about confidence that a gap is real — not about whether that gap is worth caring about. Both questions matter, and they are separate questions.
...
The practical takeaway: before running a test, decide roughly how many users you need (most experimentation tools or a sample-size calculator will do this math for you, given your current baseline conversion rate and the smallest improvement you'd care about detecting), run the test until you hit that number, and only then look at whether the result clears the significance bar. Checking before that point is where Phase 3 begins.
