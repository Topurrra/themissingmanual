...
You can now read a `.tf` file, run the loop, and explain what state is. This last phase is about the difference between *using* Terraform and using it *without scaring yourself*. Most of it is one habit and three dangers. Let's start with a panic-moment cheat-card, because some of you arrived here mid-incident.
...
**What it does in real life on a team.** The grown-up version isn't a person squinting at a terminal — it's automation. You open a pull request changing a `.tf` file; CI runs `terraform plan` and posts the output as a comment; a human reviews *that diff* before approving; and only a merge to the main branch triggers `apply`. The plan becomes a reviewable artifact, exactly like a code diff. (This is the same review habit from [Git With Other People](/guides/git-with-other-people), pointed at infrastructure.)
...
**Why people get this wrong.** The first instinct in IaC is to copy your working `web-1` block, paste it, change the name to `web-2`, and repeat. That's just click-ops with extra steps — now you have the same drift-between-copies problem, only in text. The two servers were supposed to be identical and now they differ because someone edited one copy and forgot the other.
...
*What just happened:* You described one kind of web server *once* (inside `modules/web_server/`), then asked for two of them that differ only in the inputs you chose to vary — size and environment name. Fix a security setting in the module and *both* environments get the fix on the next apply. That's the reuse click-ops can never give you, and it's why modules exist.

⚠️ **Gotcha.** Don't build a module before you have a reason. A module abstracting a single resource you use once adds indirection for no reuse — you now hop between files to read one server. Reach for a module when you're about to copy-paste the *second* time, not preemptively. (This is the "no abstractions for single-use code" instinct, applied to infrastructure.)
...
*What just happened:* Terraform noticed that the real instance is now `t3.large`, but your code still says `t3.micro` — someone resized it by hand (probably during an incident, exactly the 2am story from Phase 1). Terraform is offering to drag reality *back* to `t3.micro`. Now you have an honest decision, not a silent mystery: either that bigger size was intentional, so update your code to `t3.large` and commit it; or it was a temporary hack, so let the apply revert it. Either way, drift is now visible and resolved instead of festering.
...
- **A `-/+` replacement inside a normal `apply`** — you changed an attribute that *can't* be modified in place, so Terraform's only option is destroy-then-recreate. This is the sneaky one: you ran `apply` to make a small change and a destroy hid inside it.
...
*What just happened:* You changed the `ami` (the base image). You can't swap a running machine's underlying image in place, so Terraform reports `-/+ must be replaced` and the comment `# forces replacement` tells you exactly which attribute caused it. The summary's `1 to destroy` is the part that matters: your existing server will be **deleted** and a new one created. If that server held data on its local disk, or its IP was hard-coded somewhere, the destroy is the part that hurts.

⚠️ **Gotcha — read the summary line, every time.** `1 to add, 0 to change, 1 to destroy` looks almost like the harmless create-only plan from Phase 2. The difference is one number, and that number is a deletion. For irreplaceable resources — databases especially — Terraform supports a guard: setting `prevent_destroy = true` on the resource makes Terraform *refuse* to produce any plan that would delete it, turning a catastrophe into an error message. Use it on anything you can't afford to lose.

🪖 **War story.** The canonical Terraform horror story is a tiny change to a database resource that, because of how that attribute works, forces a replacement — and the engineer, watching the change they *intended*, types `yes` without registering the `1 to destroy`. The database is gone, recreated empty. Every word they needed was on screen. The lesson isn't "Terraform is dangerous"; it's "the plan is the safety device, and it only works if you read the destroy count."
...
**Why it happens.** Remember state is Terraform's record of what it *built*. If a resource has a password, a private key, or a generated token as one of its attributes, Terraform stores that attribute's value in state so it can detect future changes. A database resource with an initial password, for example, writes that password into `terraform.tfstate`. The state file is, by default, unencrypted JSON.
...
That's the foundation. You can now reason about Terraform instead of fearing it: declare desired state, run the loop, respect state, read every plan. The deeper craft — authoring your own modules well, multi-environment workspaces, and full CI/CD pipelines — builds directly on these four ideas, and is the subject of the follow-up guide.
