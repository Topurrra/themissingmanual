...
Before any rules about `.gitignore` or vaults, you need one idea in your head — the idea that makes every later rule feel obvious instead of arbitrary. People follow security advice badly when it's a list of memorized commandments. They follow it well when they understand what they're protecting and why it runs away. So let's install the mental model first, then name the things that count, then look honestly at how they escape.
...
The code works, the feature ships, and the key is now sitting in your repository. If that repo is public — or ever becomes public, or is cloned to a laptop that's later lost — the key is out. And because it's a literal string in the code, it's trivially findable: anyone can search for it.
...
⚠️ **Gotcha — bots are watching, and they're fast.** Public code hosts are continuously scanned by automated bots looking for exactly these patterns. A real key pushed to a public repository can be found and abused within **minutes** of the push — not days. This isn't hypothetical caution; it's the routine behavior of both attackers (hunting for free compute and data) and the good guys (GitHub's own secret-scanning service notifies you and many providers auto-revoke keys it spots). The window between "I'll fix it later" and "it's been used" can be shorter than your coffee break.

**Why this saves you later.** Once you internalize that a secret is a house key and that the fastest way to lose it is to commit it, the entire next phase reads as common sense: get the key *out* of the code, keep the file it lives in *out* of Git, and assume that if it ever got out, you must change the lock.
...
- **Logs.** Printing a request that contains an `Authorization` header, or logging the full config at startup, can write secrets into log files that get shipped to a logging service or shown in a stack trace.
- **Error pages and screenshots.** A debug error page that dumps environment variables, pasted into a ticket or a chat, leaks whatever was on screen.
- **Sharing the wrong way.** Pasting a key into a public chat, a forum question, or an issue. Once it's in a place you don't control, treat it as gone.

Each of these is the same root mistake as committing: **the secret ended up somewhere it could be seen.** Keep that single frame — "where can this value be seen, and who can see there?" — and you'll catch leaks the rules don't explicitly cover.
