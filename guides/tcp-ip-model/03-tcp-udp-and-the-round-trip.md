...
We've deferred one question twice now: at the Transport layer, your data can ride on **TCP** or **UDP**. They solve the same surface problem — get bytes to the right program — in opposite ways, and the difference shapes how everything from video calls to web pages behaves. Once you understand the trade-off, you'll know *why* a glitchy video call recovers and a glitchy file download doesn't.

Then we'll do the satisfying part: take one packet and watch it travel all the way down your stack on the way out, and all the way back up on the way in — encapsulation from [Phase 1](01-why-layers.md) and the four layers from [Phase 2](02-the-four-layers.md), finally in motion.

## TCP: reliable, ordered, connected

**What it actually is.** TCP (Transmission Control Protocol) is the careful courier. Before it sends a single byte of your data, it establishes a *connection* with the other end, and then it guarantees three things: every byte arrives, the bytes arrive *in order*, and if anything goes missing it's re-sent. Remember from [Phase 2](02-the-four-layers.md) that IP underneath makes no such promises — TCP is the layer that *builds* reliability on top of unreliable IP.
...
**What it does in real life.** When you load a web page, download a file, or send an email, you're on TCP. Those are cases where a single wrong or missing byte ruins everything — a half-downloaded program won't run, a corrupted page won't render. You want *correct* more than you want *instant*.

⚠️ **Gotcha.** All that care has a cost: the handshake adds a round trip *before* your data even starts, and waiting to re-send lost pieces adds delay. For a file download that's a fine trade. For a live video call, that same "stop and re-send the missing piece" behavior would make the call stutter and lag — which is exactly where UDP comes in.

## UDP: fast, fire-and-forget

**What it actually is.** UDP (User Datagram Protocol) is the opposite philosophy: no handshake, no connection, no guarantees. It takes your data, stamps on the ports, and throws it at the destination. If a packet gets lost, UDP neither knows nor cares. It's a postcard with no tracking versus TCP's signature-required parcel.

**What it does in real life.** This sounds reckless until you meet the cases it's *perfect* for: live video and voice calls, online games, and DNS lookups. In a video call, a packet that arrives late is *worthless* — the moment it describes already passed. You'd rather drop it and show the next frame than freeze the picture waiting for a re-send. A tiny glitch beats a long stall. That's the UDP bargain: speed and freshness over completeness.

💡 **Key point.** The choice isn't "TCP good, UDP bad." It's a trade-off matched to the job. *Does a late or missing piece ruin the result (file, page) or is it just stale (a video frame, a game position)?* If ruined → TCP. If merely stale → UDP.
...
The packet hops across the internet. Each router along the path strips and rebuilds *only the Link-layer wrapper* (because each hop is a different physical link), reads the IP header to decide the next hop, and forwards it. The TCP segment and your HTTP request inside stay sealed — no router opens them. The outer envelope changes at each hop; the inner letter never does.
...
If you've read other material, you've seen a **seven**-layer model called **OSI**, not four. Here's the short, honest version so it stops tripping you up:
