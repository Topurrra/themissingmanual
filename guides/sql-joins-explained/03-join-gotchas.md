...
You understand the join types now. This phase is about the moments where the *type* was right but the result still came out wrong — the query runs without an error, returns a number that looks plausible, and quietly lies to you. These are the bugs that ship to production because nothing turned red. The good news: all three have the same cure, a thirty-second sanity check we'll get to at the end.

We'll use the same `users` and `orders` tables from Phases 1 and 2.
...
📝 **Terminology — cartesian product / cross join.** Pairing every row of A with every row of B. If A has 3 rows and B has 4, you get 3 × 4 = 12 rows. The numbers stay small here, but on real tables — say 10,000 users and 50,000 orders — a cartesian product is 500,000,000 rows. That's how a forgotten `ON` clause locks up a database.
...
*What just happened:* With no matching rule, every user got paired with every order — Ada with order `104` (which isn't hers), Linus with all four (none are his). 3 users × 4 orders = 12 meaningless rows. The query "worked." The answer is garbage.

**The subtler version.** You don't have to omit `ON` entirely to cause this. A join condition that matches *many* rows on each side multiplies them too. If you join on a column that isn't unique — say two different order rows happen to share some value you joined on — each left row fans out across all of them. The row count balloons and you get duplicate-looking rows you didn't expect. The fix is always the same: **join on a key that uniquely identifies a row** (like `users.id`), not on a column that repeats.
...
This one is sneaky because it punishes you for doing two correct things at once. You write a LEFT JOIN to keep users with no orders — good. Then you add a `WHERE` clause to filter on something in the orders table — also reasonable. And your no-order users silently vanish, turning your LEFT JOIN back into an INNER JOIN without warning.
...
*What just happened:* Linus is gone — the very row your LEFT JOIN worked to keep. Here's why: the LEFT JOIN first produced Linus with `amount = NULL` (no orders). Then `WHERE orders.amount > 20` ran, and `NULL > 20` is **not true** (in SQL, any comparison with NULL is "unknown," which `WHERE` treats as a no). So Linus got filtered out. The `WHERE` on a right-table column quietly undid your outer join.
...
*What just happened:* Linus is back. The `amount > 20` test now happens *while* matching, so Ada keeps only her order over 20 and Linus — having no matching order at all — is still preserved with a NULL. The rule to remember: on an outer join, **conditions about the matched table go in `ON`; conditions that filter the final result go in `WHERE`.** (If you genuinely want to filter on a right column *and* drop non-matches, then an INNER join is what you actually meant — and writing it as INNER says so honestly.)

## 3. Joining on the wrong columns

The database matches whatever columns you tell it to — even when that's nonsense. It can't know that `orders.user_id` is meant to point at `users.id`; it only does what the `ON` clause says. Two ways this bites:
...
The reasoning is simple. If each order belongs to exactly one user (a many-orders-to-one-user relationship), then joining `orders` to `users` should give you **one row per matching order — never more.**
...
Everything here has been about getting the *right* rows. A different problem — a correct join that takes ages on a big table — is usually about whether the columns in your `ON` clause are indexed, and how the database chooses to execute the match. That's a whole topic of its own, and it's covered in [Why Is My Query Slow?](/guides/why-is-my-query-slow). Get the join *correct* first (this guide); make it *fast* second (that one).
