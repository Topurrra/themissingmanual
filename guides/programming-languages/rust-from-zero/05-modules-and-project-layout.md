...
Remember what `cargo new hello` made back in [Phase 1](01-install-and-first-program.md)? Let's actually
read it now that the pieces will mean something:
...
*Reading the tree:* the crate root (your `main.rs`) holds two modules; `greetings` exposes `hello` but keeps
...
`use` for something you never `mod`-declared (or never made `pub`).
...
first, [Memory & Garbage Collection](/guides/memory-and-garbage-collection) sets the stage perfectly.
