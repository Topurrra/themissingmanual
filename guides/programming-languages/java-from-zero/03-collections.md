...
```
*What just happened:* The `<String>` is a promise the **compiler enforces**. Trying to add an `int` is caught
...
💡 **Picking the right one.** Let the question pick the collection. Need **order and position** (or
...
Next we make programs *decide* and *organize logic*: `if`/`switch`, loops, and the methods that give Java
code its shape.
