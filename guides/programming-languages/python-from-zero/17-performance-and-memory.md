...
ops," two things stop being mysterious: why pushing work into a single C-implemented built-in is faster
than a hand-written loop (one C call versus thousands of trips through the eval loop), and why
...
before assuming you know where the cost is.

## The cardinal rule: measure, don't guess

Here is the single most important thing in this entire phase, and the one developers ignore most:
...
**Timing a small piece: `timeit`.** For comparing two ways of doing one small thing, `timeit` runs it
many times and reports how long. It handles the fiddly bits (warm-up, repeated runs) for you.
...
output, no side effects) and gets called with the same arguments over and over, you can have Python
remember past results instead of recomputing them. One decorator does it.
