...
Pick an entry — or let the default one auto-select after a timeout, which is what happens the other 99% of the time you boot — and the bootloader moves on to actually loading that OS.
...
Once the kernel image sits in memory and the bootloader jumps to its entry point, the bootloader's job is complete. It doesn't stick around, doesn't run alongside the kernel, doesn't get called again until the next reboot. Control passes entirely and permanently (until next boot) to the kernel, which is a genuinely different kind of program — one built to manage hardware directly rather than find and load a single file.
