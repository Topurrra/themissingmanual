...
That's not a flaw to be fixed; it's the nature of the tool. Building reliably means designing *around* these properties instead of being surprised by them. Here are the four that bite, and what to do about each.

## It's non-deterministic — the same input can give different output

**What it actually is.** A language model generates text by repeatedly picking a likely next token, and that pick involves some randomness. So the same prompt can produce different wordings — or different *answers* — on different calls. This is expected behavior, not a bug.
...
⚠️ **Gotcha.** Low temperature makes output *more* consistent — it does **not** make it guaranteed-identical or guaranteed-correct. Don't build logic that assumes the model will return the exact same string every time. If your code needs an exact, reliable value, constrain the output shape (see structured output below) and validate it, rather than trusting the model to repeat itself.

## It can be confidently wrong — hallucinations

**What it actually is.** A model can produce text that is fluent, authoritative, and *false*. It can invent a function that doesn't exist, cite a source that was never written, or state a "fact" with total confidence. This is called a **hallucination**, and it's the single most important thing to internalize: the model is optimized to produce *plausible* text, not *true* text. Plausible and true usually overlap. Usually is not always.

**What it does in real life.** The danger is precisely that wrong answers don't *look* wrong. There's no error, no flag, no lower confidence in the tone. A made-up API method reads exactly like a real one.
...
- For facts, prices, dates, or anything load-bearing — check against a real source, don't ship the model's word for it.
...
🪖 **War story.** Plenty of people have shipped a chatbot that confidently quoted a refund policy, a discount, or a legal clause that never existed — and found out the business was on the hook for what the bot promised. The model wasn't malfunctioning; it was doing exactly what it does, which is generate plausible text. The missing piece was a guardrail around it.

## It can be slow — and it can fail

**What it actually is.** A model call goes over the network to someone else's busy service and waits for text to be generated token by token. That makes it *slower and less predictable* than a typical database query or internal API. And like any network call, it can fail: the service can be briefly overloaded, rate-limit you, or time out.

**What it does in real life.** Treat the call as what it is — a network request to an external dependency that is sometimes slow and sometimes down. The habits are the ordinary ones you'd apply to any flaky external service:
...
- **Retry transient failures — carefully.** For a temporary error (overloaded, rate-limited, timed out), waiting briefly and trying again often works. Back off between attempts (wait a little longer each retry) and cap the number of tries, so a struggling service isn't hammered and your user isn't stuck.
...
**What it does in real life.** You instruct the model, in your prompt, to respond with JSON in a specific shape — and many providers offer a dedicated setting (a "JSON mode" or schema option) that strongly nudges or guarantees valid JSON. A simple extraction request might look like:
...
You can now call a hosted model, reason about its cost and limits, and build around its rough edges. The remaining craft is getting the model to actually *do what you want* — writing instructions that produce good results reliably. That's a skill of its own.
