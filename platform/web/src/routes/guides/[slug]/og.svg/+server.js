...
export async function GET({ fetch, params }) {
  ...
  const tspans = lines
    .map((l, i) => `<text x="80" y="${startY + i * 76}" fill="#131316" font-size="64" font-weight="700" font-family="IBM Plex Sans, Segoe UI, system-ui, sans-serif">${esc(l)}</text>`)
    .join('');
  ...
  ${tspans}
  <text x="80" y="560" fill="#5c5c66" font-size="28" font-family="IBM Plex Sans, Segoe UI, system-ui, sans-serif">Everything you were supposed to already know.</text>
</svg>`;
